%\PassOptionsToPackage{showframe}{geometry}
\documentclass[tesis]{tesis-usach}
\usepackage[linesnumbered, ruled, vlined, boxed, commentsnumbered, spanish, onelanguage]{algorithm2e}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% COSAS A REVISAR
% - La numeración de las páginas en la tabla de contenidos tiene algún problema con el paquete geometry. DESCRIPCIÓN DEL PROBLEMA no coincide en la página.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\usepackage{pgfplots}
\usepackage{import}
\usetikzlibrary{arrows, calc, decorations.markings, math}
\usetikzlibrary{matrix, chains, positioning, decorations.pathreplacing, shapes, snakes}


\title{Análisis de la eficiencia del entrenamiento de redes neuronales profundas basado en simulated annealing}
\informe{Informe}
\facultad{Facultad de Ingenieria}
\departamento{Departamento de ingenieria informática}

\author{Felipe Alberto Reyes González}
\programa{Magíster en Ingeniería Informática}
\profesor{Victor Parada}
\celular{890 26 317}
\correo{felipe.reyesg@usach.cl}
\date{\today}

\let\cite\shortcite
%\usepackage{layouts}
\begin{document}
\baselineskip 23pt
	\def\bibname{\uppercase{Referencias Bibliográficas}}
	%\def\bibname{REFERENCIAS BIBLIOGRÁFICAS}
	%\def\refname{REFERENCIAS BIBLIOGRÁFICAS}

	\renewcommand{\BOthers}[1]{et al.\hbox{}}%
	\maketitle
	\restoregeometry
	\makecopyright

	\frontmatter
	\indice
	\mainmatter

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% Inicio contenido
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	%\setlength{\parskip}{3mm}
	\include{contenido/capitulox01}
	\include{contenido/capitulox02}
	%\include{contenido/capitulox03}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% Fin contenido
	%%%%%%%%%%%%%%%%%%%%%%%%%%%


	%%%%%%%%%%%%%%%%%%%%%%%%
	% Bibliografía
	%%%%%%%%%%%%%%%%%%%%%%%%
	\bibliographystyle{apacite}
	\bibliography{referencias}
	%%%%%%%%%%%%%%%%%%%%%%%%
	% Fin bibliografía
	%%%%%%%%%%%%%%%%%%%%%%%%


	%%%%%%%%%%%%%%%%%%%%%%%%
	% Apéndice
	%%%%%%%%%%%%%%%%%%%%%%%%
	% \appendix
	% \clearpage
	 %\addappheadtotoc
	% \appendixpage
	%%%%%%%%%%%%%%%%%%%%%%%%
	% Fin apéndice
	%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
